...
const
{
  ID = ''//se deja valor por defecto
} =req.body; //recibe los parametros de la petición {}.

async function getReceptors(req, res) {
  const response = await ReceptorController.getReceptors(ID); //se le pasa a la funcion del modelo el parametro recibido 
  if (response.error) {
